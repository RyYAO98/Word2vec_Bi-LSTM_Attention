...
class Dataset(object):
    ...
    def _readData(self, dataSource, cross_time):
        ...
        for i in range(len(trainDataSource)):
            ...
            for j in range(5000):
                trainLabels.append([df.ix[j, 'labels']])  # 标签也须存储在二维列表中
        ...
        evalLabels = []
        for j in range(5000):
            evalLabels.append([df.ix[j, 'labels']])
...
for j in range(0, 5):
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            outDir = os.path.abspath(os.path.join("D:\\my_work\\", "summarys"))
            print("Writing to {}\n".format(outDir))
            ...
            for i in range(config.training.epoches):
                ...
                for batchTrain in nextBatch(trainReviews, trainLabels, config.batchSize):
                    ...
                    if currentStep % config.training.checkpointEvery == 0:
                        ...
                        path = saver.save(sess, "D:\\my_work\\model_ckpt_reg=3\\LSTM_Attention_" + str(cross_time) + str(currentStep) + ".ckpt")
